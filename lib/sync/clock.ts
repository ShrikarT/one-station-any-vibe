...
/** One completed round trip against the station clock. NTP's four stamps. */
export type ClockSample = {
	/** local clock when the ping left this device */
	t0: number
	/** station clock when the ping arrived */
	t1: number
	/** station clock when the reply left */
	t2: number
...
export type OffsetEstimate = {
	/** Add to a local timestamp to get station time. */
	offsetMs: number
	/** Round-trip time of the samples actually used, as a confidence signal. */
	rttMs: number
	sampleCount: number
	usedSamples: number
}
...
export type ElapsedOptions = {
	/** Track length. Without it, elapsed time is unbounded. */
	durationMs?: number
	/** When true, elapsed time wraps instead of clamping at the end. */
	loop?: boolean
}

export type PlaybackPlan = {
	/** Pass as the first argument to AudioBufferSourceNode.start(). */
	startAtContextTime: number
	/** Pass as the second argument: where to seek into the buffer. */
	offsetSeconds: number
	elapsedMs: number
	isLateJoin: boolean
}

export type PlanPlaybackInput = {
	/** Station clock timestamp at which the current track started. Externally received. */
	startedAt: number
	/** Measured offset between this device and the station. */
	offsetMs: number
	/** This device's Date.now() at the moment of planning. */
	localNowMs: number
	/** This device's AudioContext.currentTime at the same moment. */
	contextNow: number
	durationMs?: number
	loop?: boolean
...
export type DriftInput = {
	...
	offsetMs: number
	localNowMs: number
	/** Where the audio clock says this device actually is. */
...
export function sampleRoundTripMs(sample: ClockSample): number {
 ...
 */
export function estimateOffset(samples: readonly ClockSample[]): OffsetEstimate {
...
export function toStationTime(localMs: number, offsetMs: number): number {
	return localMs + offsetMs
}

/** Station timestamp to local timestamp. */
export function toLocalTime(stationMs: number, offsetMs: number): number {
 ...
 */
export function elapsedAt(startedAt: number, now: number, options: ElapsedOptions = {}): number {
	if (!isFiniteNumber(startedAt) || !isFiniteNumber(now)) return 0
	...
	if (!isFiniteNumber(durationMs) || durationMs <= 0) return raw
 ...
 */
export function planPlayback(input: PlanPlaybackInput): PlaybackPlan {
	const {
		startedAt,
		offsetMs,
		localNowMs,
		...
		durationMs,
		loop = false,
		leadMs = DEFAULT_SCHEDULE_LEAD_MS,
	} = input
 ...
 */
export function driftMs(input: DriftInput): number {
	const { startedAt, offsetMs, localNowMs, actualElapsedMs, durationMs, loop = false } = input
	if (!isFiniteNumber(actualElapsedMs)) return 0
	...
	if (loop && isFiniteNumber(durationMs) && durationMs > 0) {
		...
		while (drift > half) drift -= durationMs
		while (drift < -half) drift += durationMs
	}
